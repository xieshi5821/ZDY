...
- (void)VoiceRecognitionClientWorkStatus:(int)aStatus obj:(id)aObj
{
    switch (aStatus)
    {
        case EVoiceRecognitionClientWorkStatusFlushData: // 连续上屏中间结果
        {
            ...
            if ([[BDVoiceRecognitionClient sharedInstance] getNeedOriginJsonResultFlag])
            {//Origin Json Result:
                VRResultData *tmpResult = [[VRClientParsor sharedInstance] parseOriginJsonData:aObj];
                [tmpString appendFormat:@"%@",[tmpResult.audioResultData objectAtIndex:0]];
            }
            else
            {
